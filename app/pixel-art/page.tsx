...
const TOMASH_IMAGE =
  "https://i.ibb.co/rmc10Mt/ovodv-httpss-mj-run9g-LNTGXsh24-Happy-guy-standing-open-month-cd580a6c-5a38-44ca-9c27-3e549b9eee5b-0.png"
...
export default function PixelArtPage() {
  ...
  const initializePixels = (savedPixels: string[] | null = null) => {
    const totalPixels = GRID_SIZE * GRID_SIZE
    const initialPixels = Array(totalPixels).fill("#FFFFFF")
    ...
    // If saved pixels exist, merge them
    if (savedPixels && savedPixels.length === totalPixels) {
      savedPixels.forEach((color, index) => {
        if (color !== "#FFFFFF") {
          initialPixels[index] = color
        }
      })
    }
    ...
    try {
      ...
      const timer = setInterval(() => {
        setCountdown((prev) => {
          if (prev <= 1) {
            clearInterval(timer)
            // Redirect to the start page after countdown
      ...
      setError("Failed to save your artwork. Please try again.")
    }
  }
  ...
  if (showEndImage) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div
          className={`text-center max-w-4xl mx-auto px-4 transition-opacity duration-500 ${
            showEndImage ? "opacity-100" : "opacity-0"
          }`}
        >
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold mb-8 leading-tight">
            Thank you for your <span className="text-[#8CD6E8]">art</span>!
          </h1>
          <p className="text-2xl text-gray-600 mb-8">Returning to start in {countdown} seconds...</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div
        className={`max-w-6xl mx-auto transition-opacity duration-500 ${showEndImage ? "opacity-0" : "opacity-100"}`}
      >
        ...
        <div className="bg-white rounded-3xl shadow-lg p-8">
          ...
          <div className="flex flex-col items-center gap-8">
            ...
            <div className="flex items-center gap-8 flex-wrap justify-center">
              ...
              <Button className="bg-[#0047AB] hover:bg-[#003380] text-white px-8 py-2 text-lg" onClick={handleSave}>
                Save
              </Button>
            </div>
          </div>
